...
import { ArrowLeft, LayoutDashboard, Calendar, Receipt, Settings, Phone, User, LogOut, Home, Search, Users, Stethoscope } from "lucide-react";
...
export default function DoctorSidebarLayout({ children }: SidebarLayoutProps) {
    ...
    const navItems = [
        ...
        { name: 'Earnings', icon: Receipt, path: '/doctor/finance' },
        ...
        <div className="flex h-screen bg-gray-50 font-sans text-gray-800">
            ...
            <aside className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col shadow-xl hidden lg:flex sticky top-0 h-screen text-white">
                <div className="p-6 flex items-center justify-center border-b border-slate-800">
                    <div className="flex items-center gap-2 cursor-pointer hover:scale-105 transition-transform" onClick={() => router.push('/')}>
                        <Stethoscope className="w-8 h-8 text-emerald-400" />
                        <span className="text-xl font-bold text-white tracking-tight">Ayur<span className="text-emerald-400">Sutra</span></span>
                    </div>
                </div>
                ...
                <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-6 lg:px-8 shadow-sm shrink-0 z-10">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-2xl font-bold text-gray-800 hidden md:block">
                            Welcome Back, Dr. {user?.user_metadata?.last_name || user?.user_metadata?.first_name || 'Doctor'}
                        </h1>
                    </div>
                        ...
                        <div className="flex items-center gap-3 pl-0 lg:pl-6 lg:border-l border-gray-200">
                            <div className="text-right hidden md:block">
                                <p className="text-sm font-bold text-gray-900">
                                    Dr. {user?.user_metadata?.last_name || 'User'}
                                </p>
