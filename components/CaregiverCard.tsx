...
export function CaregiverCard({
  ...
  const handleBookAppointment = async () => {
    if (!user) {
      toast.error(
        t("auth.loginRequired", "Please sign in to book an appointment")
      );
      router.push("/sign-in");
      return;
    }
  ...
  return (
    <div
      className={`caregiver-card animate-fade-in ${className}`}
      style={style}
    >
      {/* Match Score and Distance Badges */}
      ...
      <div className="flex items-start space-x-4 mb-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center space-x-2 mb-2">
            <span
              className={`px-2 py-1 rounded-full text-xs font-medium ${getCaregiverTypeColor(
                caregiver.type
              )}`}
            >
          ...
          <div className="flex items-center space-x-2 mb-2">
            <div className="rating-stars">
              {renderStars(caregiver.rating || 0)}
            </div>
            <span className="text-sm text-gray-600">
              {(caregiver.rating || 0).toFixed(1)} (
              {caregiver.total_reviews || 0} reviews)
            </span>
          </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
          <div className="flex items-start space-x-2">
            ...
            <div className="flex-1">
              <p className="text-sm text-blue-800">{aiReason || reason}</p>
              {searchMethod && (
                <p className="text-xs text-blue-600 mt-1 opacity-75">
                  Match type: {searchMethod.replace(/_/g, " ")}
                </p>
              )}
            </div>
          </div>
